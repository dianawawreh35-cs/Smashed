...
| R-11 | Missed calls: calls not answered by an agent, and calls abandoned in the queue (see 4.5) — count, list, and time until the customer was called back (or "never"). Calls the agents' apps saw are immediate; abandoned calls come from the CDR import and appear within its interval (S-55). *Amended 26 Sep 2026 (Dia): built as the count, the rate against incoming calls and the list of missed calls. The time until called back is deferred with the abandoned calls it belongs to, until the CDR import exists.* |
...
**As built (26 Sep 2026):** R-01 to R-18 are on one page, **Call reports** (تقارير المكالمات), right under Calls, in six tabs: Overview, Orders and channels, Customers, Agents, Problems, Data quality. R-02 is the Calls page and its full export. R-19, R-20 and R-21 are not built: R-19 is a *Could*, and R-20 and R-21 need the CDR import (S-55), so they are left off until it exists rather than shown empty. The reports count **calls only, except where the report is the comparison** — R-01's calls vs app and R-12 to R-14 count application messages as well (Dia, 25 Sep). The words they use:
