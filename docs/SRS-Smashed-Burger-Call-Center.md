...
| ID | Requirement | Priority |
|---|---|---|
| S-40 | Edit the classification forms used by agents: add/remove/rename types; add custom fields (text, number, dropdown, checkbox); set required fields; the change applies to new classifications without reinstalling the Agent App. **There are two forms, one for incoming calls and one for outgoing calls**, edited on the same page and published separately; the call types are shared between them. | Must |
